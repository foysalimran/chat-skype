	...
	class SCS_Field_spacing extends SCS_Fields {
		...
		public function render() {
			...
			if ( ! empty( $args['all'] ) ) {
				...
				echo '<input type="number" name="' . esc_attr( $this->field_name( '[all]' ) ) . '" value="' . esc_attr( $value['all'] ) . '"' . wp_kses_post( $placeholder ) . ' class="scs-input-number' . esc_attr( $is_unit ) . '" step="any" />';
				...
				foreach ( $properties as $property ) {
					...
					echo '<input type="number" name="' . esc_attr( $this->field_name( '[' . $property . ']' ) ) . '" value="' . esc_attr( $value[ $property ] ) . '"' . wp_kses_post( $placeholder ) . ' class="scs-input-number' . esc_attr( $is_unit ) . '" step="any" />';
					echo ( $unit ) ? '<span class="scs--label scs--unit">' . esc_attr( $args['units'][0] ) . '</span>' : '';
					echo '</div>';
