 ...
 * @version 1.0.0
 */
if ( ! class_exists( 'SCS_Field_link_color' ) ) {
	class SCS_Field_link_color extends SCS_Fields {
		...
		public function render() {
			...
			foreach ( $color_props as $color_prop_key => $color_prop_value ) {
				...
				if ( ! empty( $args[ $color_prop_key ] ) ) {
					...
					echo '<input type="text" name="' . esc_attr( $this->field_name( '[' . $color_prop_key . ']' ) ) . '" value="' . esc_attr( $value[ $color_prop_key ] ) . '" class="scs-color"' . wp_kses_post( $default_attr ) . wp_kses_post( $this->field_attributes() ) . '/>';
					echo '</div>';
