 ...
 * @version 1.0.0
 */
if ( ! class_exists( 'SCS_Field_button_set' ) ) {
	class SCS_Field_button_set extends SCS_Fields {
		...
		public function render() {
			...
			if ( isset( $this->field['options'] ) ) {
				...
				if ( is_array( $options ) && ! empty( $options ) ) {
					...
					echo '<div class="scs-siblings scs--button-group" data-multiple="' . esc_attr( $args['multiple'] ) . '">';

					foreach ( $options as $key => $option ) {

						$type    = ( $args['multiple'] ) ? 'checkbox' : 'radio';
						$extra   = ( $args['multiple'] ) ? '[]' : '';
						$active  = ( in_array( $key, $value ) || ( empty( $value ) && empty( $key ) ) ) ? ' scs--active' : '';
