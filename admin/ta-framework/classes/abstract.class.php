 ...
 * @version 1.0.0
 */
if ( ! class_exists( 'SCS_Abstract' ) ) {
	abstract class SCS_Abstract {
		...
		public function __construct() {
			...
			if ( ! empty( $this->args['output_css'] ) || ! empty( $this->args['enqueue_webfont'] ) ) {
				...
				SCS::$css = apply_filters( "scs_{$this->unique}_output_css", SCS::$css, $this );
			}
		}
		...
		public function recursive_output_css( $fields = array(), $combine_field = array() ) {
			...
			if ( ! empty( $fields ) ) {
				...
				foreach ( $fields as $field ) {
					...
					$field_class  = 'SCS_Field_' . $field_type;
