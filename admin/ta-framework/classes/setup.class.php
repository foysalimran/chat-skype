...
if ( ! class_exists( 'SCS_Setup' ) ) {
  class SCS_Setup {
    ...
    public function __construct() {
      ...
      add_action( 'after_setup_theme', array( 'SCS', 'setup' ) );
      add_action( 'init', array( 'SCS', 'setup' ) );
      add_action( 'switch_theme', array( 'SCS', 'setup' ) );
      add_action( 'admin_enqueue_scripts', array( 'SCS', 'add_admin_enqueue_scripts' ) );
      add_action( 'wp_enqueue_scripts', array( 'SCS', 'add_typography_enqueue_styles' ), 80 );
      add_action( 'wp_head', array( 'SCS', 'add_custom_css' ), 80 );
      add_filter( 'admin_body_class', array( 'SCS', 'add_admin_body_class' ) );
    ...
    public static function setup() {
      ...
      $params = array();
      if ( class_exists( 'SCS_Options' ) && ! empty( self::$args['admin_options'] ) ) {
        foreach ( self::$args['admin_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            SCS_Options::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'SCS_Customize_Options' ) && ! empty( self::$args['customize_options'] ) ) {
        foreach ( self::$args['customize_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            SCS_Customize_Options::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'SCS_Metabox' ) && ! empty( self::$args['metabox_options'] ) ) {
        foreach ( self::$args['metabox_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            SCS_Metabox::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'SCS_Nav_Menu_Options' ) && ! empty( self::$args['nav_menu_options'] ) ) {
        foreach ( self::$args['nav_menu_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            SCS_Nav_Menu_Options::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'SCS_Profile_Options' ) && ! empty( self::$args['profile_options'] ) ) {
        foreach ( self::$args['profile_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            SCS_Profile_Options::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'SCS_Taxonomy_Options' ) && ! empty( self::$args['taxonomy_options'] ) ) {
        ...
        foreach ( self::$args['taxonomy_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            SCS_Taxonomy_Options::instance( $key, $params );
      ...
      if ( class_exists( 'SCS_Widget' ) && class_exists( 'WP_Widget_Factory' ) && ! empty( self::$args['widget_options'] ) ) {
        ...
        foreach ( self::$args['widget_options'] as $key => $value ) {
          if ( ! isset( self::$inited[$key] ) ) {
            ...
            self::$inited[$key] = true;
            $wp_widget_factory->register( SCS_Widget::instance( $key, $value ) );
      ...
      $params = array();
      if ( class_exists( 'SCS_Comment_Metabox' ) && ! empty( self::$args['comment_options'] ) ) {
        foreach ( self::$args['comment_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            SCS_Comment_Metabox::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'SCS_Shortcoder' ) && ! empty( self::$args['shortcode_options'] ) ) {
        foreach ( self::$args['shortcode_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            SCS_Shortcoder::instance( $key, $params );

          }
        }

        // Once editor setup for gutenberg and media buttons
        if ( class_exists( 'SCS_Shortcoder' ) && ! empty( self::$shortcode_instances ) ) {
          foreach ( self::$shortcode_instances as $instance ) {
            if ( ! empty( $instance['show_in_editor'] ) ) {
              SCS_Shortcoder::once_editor_setup();
              break;
            }
          }
        }
    ...
    public static function include_plugin_file( $file, $load = true ) {
      ...
      $path     = '';
      $file     = ltrim( $file, '/' );
      $override = apply_filters( 'scs_override', 'scs-override' );
    ...
    public static function includes() {
      ...
      $fields = apply_filters( 'scs_fields', array(
        'accordion',
        'background',
        'backup',
        'border',
      ...
      if ( ! empty( $fields ) ) {
        foreach ( $fields as $field ) {
          if ( ! class_exists( 'SCS_Field_'. $field ) && class_exists( 'SCS_Fields' ) ) {
    ...
    public static function add_admin_enqueue_scripts() {
      ...
      if ( ! self::$enqueue ) {
        ...
        if ( $wpscreen->id === 'tools_page_scs-welcome' ) {
          self::$enqueue = true;
        }

      }

      if ( ! apply_filters( 'scs_enqueue_assets', self::$enqueue ) ) {
        return;
      }
      ...
      if ( apply_filters( 'scs_fa4', false ) ) {
        wp_enqueue_style( 'font-awesome_v4-fa', SCS_DIR_URL . 'admin/ta-framework/assets/css/font-awesome_v4.min.css', array(), '4.7.0', 'all' );
      } else {
        wp_enqueue_style( 'font-awesome_v5-fa5', SCS_DIR_URL .'admin/ta-framework/assets/css/font-awesome_v5.min.css', array(), '5.15.5', 'all' );
        wp_enqueue_style( 'font-awesome_v4-v4-shims', SCS_DIR_URL .'admin/ta-framework/assets/css/font-awesome_v4-shims.min.css', array(), '5.15.5', 'all' );
      }
      ...
      // Main style
      wp_enqueue_style( 'scs', self::include_plugin_url( 'assets/css/style'. $min .'.css' ), array(), self::$version, 'all' );

      // Main RTL styles
      if ( is_rtl() ) {
        wp_enqueue_style( 'scs-rtl', self::include_plugin_url( 'assets/css/style-rtl'. $min .'.css' ), array(), self::$version, 'all' );
      }

      // Main scripts
      wp_enqueue_script( 'scs-plugins', self::include_plugin_url( 'assets/js/plugins'. $min .'.js' ), array(), self::$version, true );
      wp_enqueue_script( 'scs', self::include_plugin_url( 'assets/js/main'. $min .'.js' ), array( 'scs-plugins' ), self::$version, true );

      // Main variables
      wp_localize_script( 'scs', 'scs_vars', array(
        'color_palette'     => apply_filters( 'scs_color_palette', array() ),
        'i18n'              => array(
      ...
      if ( ! empty( self::$fields ) ) {
        foreach ( self::$fields as $field ) {
          if ( ! empty( $field['type'] ) ) {
            $classname = 'SCS_Field_' . $field['type'];
    ...
    public static function add_typography_enqueue_styles() {
      ...
      if ( ! empty( self::$webfonts ) ) {
        ...
        if ( ! empty( self::$webfonts['enqueue'] ) ) {
          ...
          wp_enqueue_style( 'scs-google-web-fonts', esc_url( add_query_arg( $query, '//fonts.googleapis.com/css' ) ), array(), null );
        ...
        if ( ! empty( self::$webfonts['async'] ) ) {
          ...
          wp_enqueue_script( 'scs-google-web-fonts', esc_url( '//ajax.googleapis.com/ajax/libs/webfont/1.6.26/webfont.js' ), array(), null );

          wp_localize_script( 'scs-google-web-fonts', 'WebFontConfig', array( 'google' => array( 'families' => $fonts ) ) );
    ...
    public static function field( $field = array(), $value = '', $unique = '', $where = '', $parent = '' ) {
      ...
      $is_pseudo  = ( ! empty( $field['pseudo'] ) ) ? ' scs-pseudo-field' : '';
      ...
      if ( ! empty( $field['dependency'] ) ) {
        ...
        $visible = ( ! empty( $depend_visible ) ) ? ' scs-depend-visible' : ' scs-depend-hidden';
      ...
      if ( ! empty( $field_type ) ) {
        ...
        echo ( ! empty( $field['title'] ) ) ? '<div class="scs-fieldset">' : '';

        $value = ( ! isset( $value ) && isset( $field['default'] ) ) ? $field['default'] : $value;
        $value = ( isset( $field['value'] ) ) ? $field['value'] : $value;

        $classname = 'SCS_Field_'. $field_type;
 ...
 *
 */
if ( ! class_exists( 'chat-skype' ) ) {
  class SCS extends SCS_Setup{}
}
